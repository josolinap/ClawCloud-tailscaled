...
FROM alpine:3.20
RUN apk add --no-cache ca-certificates tini supervisor python3 iptables
...
RUN mkdir -p /var/run/tailscale /var/lib/tailscale /workspace
...
# Healthcheck with longer start period to allow ephemeral startup
